...
=head1 SYNOPSIS
    ...
    # autobox_can() and autobox_isa() work like can() and isa()

        if ("Hello, world!"->autobox_can('foo')) ...
        if (3.1415927->autobox_isa('SCALAR')) ...
...
Although C<autobox> doesn't export anything, it includes an additional module, C<autobox::universal>,
which exports a single subroutine, C<type>.
...
=head3 VERSION, import and unimport

Although C<autobox_can> and C<autobox_isa> work as expected for autoboxed values, the C<VERSION> method doesn't.
Thus, while these work:

    [ ... ]->autobox_can('pop')

    3.1415->autobox_isa('MyScalar')
